...
|---|---|---|---|
| zebrafish chambers (Fig. 2c–f) | 10 µm/px, 600 volumes s⁻¹, 20-µm planes | 5.616 µm/px | areas and dV/dt × (5.616/10)² = 0.3154 (`source_data/rescale_source_data.py`) |
...
| Supplementary Videos | bars drawn for 128-px frames (11 / 30.3 µm/px) | 5.616 / 17.0 µm/px (256-px frames) | bars redrawn with `figures_videos/fix_video_overlays.py`; the presets in `Depth_cycle_video_gui.py` are now the 256-px values |
| mouse paw (Supplementary Fig. 7) | 2.93 µm/px passed with `--px-um` (script fallback 4.0) | 2.808 µm/px | scale bars differ by 4 % |

The GUI defaults of `mouse_ear/ROI_analysis_*.py` (17.0 µm/px, 20 volumes s⁻¹) and the CONFIG of
`zebrafish_heart/analyze_heart_ver6.py` (600 volumes s⁻¹, 5.616 µm/px, 20-µm planes) were set to the values
